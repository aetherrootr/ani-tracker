...
| `TVDB_PIN` | 可选的 TheTVDB API PIN。需要通过 TheTVDB 订阅自行获取，ani-tracker 不提供。 |
| `ANIME_SYNC_CRON_HOUR` | 实现 `get_updates` 的供应商执行完整校准的小时列表，默认为 `4`。 |
| `ANIME_SYNC_CRON_MINUTE` | 增量供应商完整校准的分钟，默认为 `0`。 |
| `NON_INCREMENTAL_ANIME_SYNC_CRON_HOUR` | 未实现 `get_updates` 的供应商完整同步小时列表，默认每四小时一次。 |
| `NON_INCREMENTAL_ANIME_SYNC_CRON_MINUTE` | 非增量供应商完整同步的分钟，默认为 `0`。 |
| `EPISODE_STATUS_REFRESH_INTERVAL_SECONDS` | 本地 upcoming 到 aired 状态翻转间隔，默认为 `60` 秒。 |
| `PROVIDER_UPDATES_ENABLED` | 是否轮询所有实现 `get_updates` 的已配置供应商，默认启用。 |
| `PROVIDER_UPDATES_INTERVAL_SECONDS` | 供应商 updates 轮询间隔，默认为 `900` 秒。 |
